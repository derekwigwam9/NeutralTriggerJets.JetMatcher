...
// filepaths
static const TString SOutDefault("pp200py.resTestResponse.r02a005rm1chrg.root");
...
static const UInt_t NHadIds(17);
...
void MatchPythiaJets(const TString pPath=SParDefault, const TString dPath=SDetDefault, const TString oPath=SOutDefault, Bool_t inBatchMode=false) {
  ...
  const UInt_t   idHadTrg[NHadIds] = {8, 9, 11, 12, 14, 15, 19, 21, 23, 24, 27, 29, 31, 32, 45, 46, 111};
  ...
  TTree *tDet;
  if (fPar && fDet) {
    fPar -> GetObject("femtoDST", tPar);
    fDet -> GetObject("femtoDST", tDet);
  }
  ...
  for (UInt_t i = 0; i < nEvts; i++) {
    ...
    const Bool_t isGoodParTrg  = (isInEtParCut && isInEtaParCut && isHadronPar);
    ...
    const UInt_t   tspTrgD = (UInt_t) dTSP;

    Bool_t isHadronDet(false);
    for (UInt_t iHad = 0; iHad < NHadIds; iHad++) {
      if (tspTrgD == idHadTrg[iHad]) {
        isHadronDet = true;
        break;
      }
    }
    ...
    const Bool_t isGoodDetTrg  = (isInEtaDetCut && isInEtDetCut && isHadronDet);
    ...
    hNumJetsP   -> Fill(nPjets);
    hNumJetsD   -> Fill(nDjets);
