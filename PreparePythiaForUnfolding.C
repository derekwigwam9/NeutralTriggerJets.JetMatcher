...
void PreparePythiaForUnfolding() {
  ...
  // io parameters
  const TString sOut("pp200py.resTestResponse.pTbinRes.et920pi0.r05a065rm1chrg.dr05q015185.root");
  const TString SIn("pp200py.resTestResponse.r05a065rm1chrg.dr05q015185.root");
  ...
  for (UInt_t iBinY = 0; iBinY < nYbins; iBinY++) {
    ...
    if (binNorm != 0.) {
      for (UInt_t iBinX = 0; iBinX < nXbins; iBinX++) {
        ...
        const Double_t dArea  = xWidth * yWidth;
        const Double_t newVal = (binVal / binNorm)/* * dArea*/;
        const Double_t newErr = (binErr / binNorm)/* * dArea*/;
